...
export class GetPageUseCase extends BaseUseCase<
...
> {
  ...
  async beforeExecute(input: {
    ...
    if (input.pageId) ParamIdValidationSchema.parse(input.pageId);
    else
      z.string({
  ...
  async execute(
    ...
    context?: AuthContext,
  ): Promise<UsecaseResult<Page>> {
    try {
      let entity: Page;
      if (input.pageId) {
        entity = await this.pageRepository.findById(input.pageId);
        const sections = await this.pageRepository.getAllSections(input.pageId);
      ...
      logger.error(this.constructor.name.concat(":Failed"), {
        error,
        input,
        context,
      });
