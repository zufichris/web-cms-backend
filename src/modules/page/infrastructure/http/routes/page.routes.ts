...
export function createPageRouter(controller: PageController, authMiddleware: ReturnType<typeof createAuthMiddleware>): Router {
    ...
    router.route('/')
        .get(authMiddleware.authenticate, authMiddleware.authorizeOwnerOrAdmin("ownerId"), controller.getAll)
        .post(authMiddleware.authenticate, authMiddleware.authorizeOwnerOrAdmin("ownerId"), controller.getAll, controller.create);
