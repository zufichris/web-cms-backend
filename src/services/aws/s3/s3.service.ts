...
export class S3Service {
    ...
    constructor({ bucketName, region, accessKeyId, secretAccessKey }: { bucketName: string, region: string, accessKeyId?: string, secretAccessKey?: string }) {
    ...
    async uploadFile(
        ...
        try {
            ...
            logger.info(`File uploaded successfully to S3: ${fileKey}, ETag: ${data.ETag}`);
            return {
                eTag: data.ETag,
    ...
    async listFiles(
    ...
    ): Promise<{ files: S3File[]; nextContinuationToken?: string; isTruncated?: boolean }> {
        ...
        try {
            ...
            const files: S3File[] = (data.Contents || []).map((item: S3Object) => ({
                ...
                size: item.Size,
                url: `${env.in_prod ? env.url_prod : env.url_dev}/api/v1/files/${item.Key}`,
            }));
            console.log(files[0])

            logger.info(`Listed ${files.length} files from S3 with prefix: '${prefix || ""}'`);
            return {
                files,
            ...
            logger.error(`Error listing files from S3 with prefix '${prefix || ""}':`, error);
            this.handleError(error)
        }
    }
